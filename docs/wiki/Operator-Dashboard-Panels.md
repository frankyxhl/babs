...
A batch of operator-facing dashboard panels for Babs, scoped to the
**self-evolution flywheel operated from a phone**: watch Citizens build Babs,
give them durable standing context, and review/approve their code changes —
including on mobile. The panel set is **inspired by prior art in existing
multi-agent dashboards**; this batch is a clean-room, BEAM-native design for Babs.
...
Babs already has the core (Citizens, terminal/PTY, multi-CLI, Tickets +
...
phone. These are the cheap, high-leverage panels; heavier machinery (a general
plugin/extension system, a unified event bus, diagram/skill editors, external IM
...
A general `Babs.Extension.Registry` plugin system (the runtime-adapter case is
already solved by `provider_runtime` + `direct_cli/adapter`), a unified
`Babs.EventLog` / event bus, a diagram editor, a skill editor, notebook,
glossary, BDD-in-UI, and external IM adapters. Phase 2–4 panels are built so a
future registry can absorb them.
