...
`next_d` = **D2** (next new topic gets D2)
...
|----|-------|--------|-------|
| D1 | Phase 5 preparation | Active | Phase 4 PR #12 is merged. `BAB-2214` PRP approved after Trinity fast-review R2 with GLM and DeepSeek. `BAB-2215` CHG approved after Trinity fast-review strict `COR-1602` + `COR-1609`: GLM 9.49/10, DeepSeek 9.0/10. Phase 5 is implemented locally on `codex/phase-5-prep`: `/citizens` index, root redirect, compact terminal tabs, preserved `?full=1`, socket-token-preserving links, `StatusSnapshot`, and browser-harness Phase 5 multi-Citizen index/tab/fd BDD. Local validation passed: ExUnit/coverage (`:babs_citizens` 83.59%, `:babs` 84.10%), JS tests, browser-harness BDD, Playwright smoke, Gate A, Alfred validation, and whitespace check. Trinity implementation fast-review passed with GLM 9.19/10 and DeepSeek 9.38/10. GitHub Codex PR review R1/R2/R3 findings were fixed: terminal tabs now only link pane-backed Citizens, non-live index Open/Full controls are disabled, malformed non-string socket tokens are ignored, tabbed terminals load LiveView, terminal-owned connection status survives LiveView patches, and active non-up tab status is preserved. PR review loop continues. The 30 minute fd stability run stays deferred to Phase 6/M2. |
...
- Continue Phase 5 from PR creation and PR review.
