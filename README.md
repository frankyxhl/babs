...
Babs is a multi-agent runtime that hosts citizens (`*.bob/`), relays messages across Discord/Telegram/Web/tmux, and coordinates agent-to-agent (A2A) communication on a single BEAM node or across a Tailscale mesh.
...
Both are named after Bat-Family support roles. Alfred Pennyworth keeps the runbook for one Batman; Babs (Barbara Gordon) coordinates the whole Bat-Family from her clocktower. The phonetic resonance with `*.bob/` citizen directories is intentional — Babs takes care of the Bobs.
